	...
	public static func evaluate(call: FunctionCall, surface: SurfaceState) -> Any? {
		...
		switch call.call {
			...
			case "formatDate":
				...
				let tzIdentifier = resolvedArgs["timeZone"] as? String ?? TimeZone.autoupdatingCurrent.identifier
				let tz = TimeZone(identifier: tzIdentifier) ?? .autoupdatingCurrent
				let localeIdentifier = resolvedArgs["locale"] as? String ?? Locale.autoupdatingCurrent.identifier
				let locale = Locale(identifier: localeIdentifier)
				return formatDate(value: val, format: format, timeZone: tz, locale: locale)
			case "pluralize":
				guard let val = asDouble(resolvedArgs["value"]),
